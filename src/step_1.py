...
def _layerize_edges(edges):
    def norm(u, v): 
        return (u, v) if u <= v else (v, u)

    remaining = set(norm(u, v) for (u, v) in edges)
    layers = []

    while remaining:
        H = nx.Graph()
        H.add_edges_from(remaining)
        M = nx.algorithms.matching.maximal_matching(H) 
        layer = [norm(u, v) for u, v in M]
    ...
    return [edge for layer in layers for edge in layer]


def build_ansatz(ansatz: str, ansatz_params: dict, num_qubits: int, backend: BackendV2) -> tuple[QuantumCircuit, dict | None]:
    
    def find_small_world_edges(N=num_qubits, k=2, samples_per_p=50, max_connected_tries=200):

        threshold = math.log(N) / math.log(k)
        best_result = {'p': None, 'avg_path_length': float('inf')}
        for p in [0.1]:
            for s in range(samples_per_p):
                seed = (s + 1) * 1009
                G = nx.connected_watts_strogatz_graph(N, k, p, tries=max_connected_tries, seed=seed)
                L = nx.average_shortest_path_length(G)
                # Check if the average path length is smaller than the threshold
                if L < threshold:
                    edges = list(G.edges())
                    return edges  # immediately return and exit the function

            # Update best result with the lowest average path length
            if L < best_result['avg_path_length']:
                best_result['p'] = p
                best_result['avg_path_length'] = L
                best_result['edges'] = list(G.edges())  # Store edges of the best graph

        # If no small-world graph was found, return the best result and its edges
        return best_result['edges']

    def apply_entanglement_map(entanglement):
        # custom-constructed
        if entanglement == 'bilinear':
            # Add coupling for first and last qubits
            entanglement_pairs = [[i, i+1] for i in range(0, num_qubits-1, 2)] + [[i, i+1] for i in range(1, num_qubits-1, 2)]
            return entanglement_pairs, None
        if entanglement == 'smallworld':
            # Add coupling for first and last qubits
            entanglement_pairs = _layerize_edges(find_small_world_edges(N=num_qubits, k=2, samples_per_p=50, max_connected_tries=200))
            return entanglement_pairs, None
    ...
    if ansatz == 'TwoLocalxx':
        rxx_params = ParameterVector(name='p',length=1)
        entanglement_block = QuantumCircuit(2)
        entanglement_block.rxx(rxx_params[0], 0, 1)
        ansatz_params_ = {'rotation_blocks':'ry', 'entanglement_blocks':entanglement_block, 'entanglement': 'bilinear', 'reps': 1, 'overwrite_block_parameters': True, 'flatten': True,'skip_final_rotation_layer': True}
