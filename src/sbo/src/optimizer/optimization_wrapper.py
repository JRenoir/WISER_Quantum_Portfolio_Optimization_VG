...
from qiskit.algorithms.optimizers import QNSPSA #new dependence
from .QNSPSA_extra import qnspsa_params #new dependence
from .optimization_monitor import callback #new dependence
...
def run(  # pylint: disable=too-many-positional-arguments
    ...
    if method == "nft":
        ...
        #new branch for QNSPSA
        method = QNSPSA
        solver_options = qnspsa_params()
        LOGGER.info("Using parameters: maxiter %s", str(solver_options['maxiter']))
        LOGGER.info("Using parameters: learning_rate %s", 'auto' if solver_options['learning_rate'] is None else str(solver_options['learning_rate']))
        LOGGER.info("Using parameters: resamplings %s", str(solver_options['resamplings']))
